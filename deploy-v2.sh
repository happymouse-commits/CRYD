#!/bin/bash
# ============================================================
# CRYD v2 自动部署脚本（新版，端口 8081）
# 仓库: github.com/happymouse-commits/CRYD-v2
# 由 ECS cron 定时执行，或手动运行
# 用法: bash deploy-v2.sh
# ============================================================
set -e

REPO_DIR="/root/SGHR-v2"
FRONTEND_DIR="$REPO_DIR/frontend"
BACKEND_DIR="$REPO_DIR/后端"
DEPLOY_TIME=$(date +%Y%m%d_%H%M%S)
JAR_PATH="/opt/sghr-v2/app/cryd-1.0.0.jar"
BACKUP_DIR="/opt/sghr-v2/app/backups"
FRONTEND_TARGET="/opt/sghr-v2/frontend"
LOG_FILE="/opt/sghr-v2/app/deploy.log"
...
# ---- 7. 重启服务 ----
log "[7/8] 重启 Spring Boot (v2)..."
systemctl restart sghr-v2
sleep 3
log "   服务已重启"

# ---- 8. 健康检查 ----
log "[8/8] 健康检查 (端口 8081)..."
SUCCESS=false
